...
Page({
  ...
  bindSave: function (e) {
    ...
    wx.showModal({
      title: '成功',
      content: '信息保存成功，管理人员稍后与你联系，或直接联系管理人员：15665259705',
      showCancel: false
    })
  ...
  onShowadress: function (_this) {

    // console.log(id);
    if (_this.data.id) { // 修改初始化数据库数据
      WXAPI.addressDetail(_this.data.id, wx.getStorageSync('token')).then(function (res) {
        console.log(res);
        if (res.code === 0) {
          _this.setData({
            id: _this.data.id,
